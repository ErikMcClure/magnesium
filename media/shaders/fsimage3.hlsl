...
Texture2D<float4> diffuse3 : register(t2);
SamplerState samp3 : register(s2);

float4 mainPS(PS_INPUT input) : SV_Target
{ 
  return diffuse3.Sample(samp3, input.t3)*diffuse2.Sample(samp2, input.t2)*diffuse.Sample(samp, input.t)*input.color;
}