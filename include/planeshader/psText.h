...
namespace planeshader {
  ...
  public:
    ...
    psText(psText&& mov);
    explicit psText(psTexFont* font=0, const char* text = 0, float lineheight = 0.0f, const psVec3D& position = VEC3D_ZERO, FNUM rotation = 0.0f, const psVec& pivot = VEC_ZERO, psFlag flags = 0, int zorder = 0, psStateblock* stateblock = 0, psShader* shader = 0, psPass* pass = 0, const psVec& scale = VEC_ONE);
    ~psText();
