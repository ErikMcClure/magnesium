...
namespace planeshader {
  ...
  struct BSS_COMPILER_DLLEXPORT psEllipseT
  {
    ...
    inline bool BSS_FASTCALL IntersectCircle(const VEC& pos, T R) const { return psCircleT<T>::CircleEllipseIntersect(x, y, a, b, pos.x, pos.y, R); }
    ...
    inline bool BSS_FASTCALL IntersectEllipse(const psEllipseT<T>& other) const { return IntersectEllipse(other.x, other.y, other.a, other.b); }
    ...
    inline void BSS_FASTCALL NearestPoint(const VEC& pos, T& outX, T& outY) const { EllipseNearestPoint(a, b, pos.x-x, pos.y-y, outX, outY); outX+=x; outY+=y; }
